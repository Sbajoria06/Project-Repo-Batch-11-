...
Audio Input
Podcast episodes in MP3 format are used as input.

Speech-to-Text
Whisper converts audio into timestamped transcripts.

Preprocessing
Transcript cleaning and sentence tokenization are performed.

Sentence Embedding
Each sentence is converted into a semantic vector using MiniLM.

Topic Segmentation
Cosine similarity between adjacent sentence embeddings detects topic shifts.

Threshold logic:
threshold = np.mean(similarities) - SIMILARITY_OFFSET

Summary Generation
Extractive summarization selects the highest TF-IDF scoring sentence.

Keyword Extraction
Top 5 TF-IDF terms are extracted per segment.

Sentiment Analysis
VADER classifies segments as Positive, Negative, or Neutral.
